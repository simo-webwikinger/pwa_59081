<template>
  <div class="border border-neutral-200 rounded-md hover:shadow-lg flex flex-col text-white" data-testid="product-card">
    ...
    <div class="p-2 border-t border-neutral-200 typography-text-sm flex flex-col flex-auto">
      <SfLink :tag="NuxtLink" :to="productPath" class="no-underline hover:!text-secondary-600" variant="secondary">
        {{ name }}
      </SfLink>
      <div class="flex items-center pt-1 gap-1" :class="{ 'mb-2': !productGetters.getShortDescription(product) }">
        <SfRating class="!text-slate-200" size="xs" :half-increment="true" :value="rating ?? 0" :max="5" />
        <SfCounter size="xs">{{ ratingCount }}</SfCounter>
      </div>
      <div
        v-if="productGetters.getShortDescription(product)"
        class="block py-2 font-normal typography-text-xs text-slate-200 text-justify whitespace-pre-line break-words"
      >
      ...
      <div class="flex flex-col-reverse items-start md:flex-row md:items-center mt-auto">
        ...
        <span v-if="crossedPrice" class="typography-text-sm text-slate-200 line-through md:ml-3 md:pb-2">
